...
while True:
    ...
    F = np.zeros((N,3))
    dBx = field_data[nearest_idxs[in_bounds] + 441, 3] - field_data[nearest_idxs[in_bounds] - 441, 3] # Difference in Bx values. Shape should be (M,1) where M is len(in_bounds)
    dBy = field_data[nearest_idxs[in_bounds] + 21, 4] - field_data[nearest_idxs[in_bounds] - 21, 4]
    dBz = field_data[nearest_idxs[in_bounds] + 1, 5] - field_data[nearest_idxs[in_bounds] - 1, 5]

    Fx = mu*spins[in_bounds, 0]*dBx / 0.01 # mu_x * dBx. Should be shape (M,1) where M is len(in_bounds)
    Fy = mu*spins[in_bounds, 1]*dBy / 0.01
    Fz = mu*spins[in_bounds, 2]*dBz / 0.01
    ...
    k = (Bvec.transpose() / B).transpose() # Transpose so that the i'th neutron divides all components by B[i]

    # Update angle change at the end of the step
...
print(anglediffs[:5, :5])
sys.exit()
print("Max Angle Differences:")
print(np.max(anglediffs[:, :numsteps-2], axis=1))
